...
namespace Phi.MobileWebApp.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        private PhiUserManager _userManager;
        private readonly IUserEmailStore<PhiUser> _emailStore;
        ...
        public PhiUserManager UserManager
        {
            get
            {
                return _userManager ?? HttpContext.GetOwinContext().GetUserManager<PhiUserManager>();
            }
            private set
            {
                _userManager = value;
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    user.LastLoggedDate = DateTime.UtcNow;
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new PhiUser() { UserName = model.Email, Email = model.Email, DateCreated = DateTime.UtcNow };
                ...
                if (result.Succeeded)
                {
                    ...
                    await UserManager.SendEmailAsync(user.Id,
                        ...
                        String.Format(GlobalResources.ConfirmYourAccountContent, callbackUrl, "support@cleverwearer.com"),
                        String.Format(GlobalResources.ConfirmYourAccountContentPlainText, callbackUrl, "support@cleverwearer.com"));

                    await UserManager.SendEmailAsync(null, "Новый пользователь", "Новый пользователь пришёл! Ура! User: " + (user.Email ?? user.Id.ToString()));
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                await UserManager.SendEmailAsync(user.Id,
                        GlobalResources.ResetYourPasswordSubject,
                        String.Format(GlobalResources.ResetYourPasswordContent, callbackUrl),
                        String.Format(GlobalResources.ResetYourPasswordContentPlainText, callbackUrl));
        ...
        public async Task<ActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                MyIdentityResult result = await UserManager.ResetPasswordAsync(user.Id, model.Code, model.Password);
                if (result.IsSucceeded)
                {
        ...
        private async Task SignInAsync(PhiUser user, bool isPersistent)
        {
            ...
            AuthenticationManager.SignIn(new AuthenticationProperties() { IsPersistent = isPersistent }, await PhiUserHelpers.GenerateUserIdentityAsync(user, UserManager));
        }
        ...
        private void AddErrors(MyIdentityResult result)
        {
            foreach (var error in result.Errors)
            {
                ModelState.AddModelError("", error);
